...
The technician portal supports offline operation. Key points for debugging:

- **Write queue** (`lib/offline/writeQueue.ts`): Queues `accept_job`, `start_job`, `complete_job` actions to IndexedDB when the device is offline. Auto-drains when connectivity returns via the `useWriteQueueSync` hook mounted in `TechnicianLayoutClient`.
- **OfflineQueueContext** (`contexts/OfflineQueueContext.tsx`): Provides `pendingCount` (number of queued actions) and `retryAll()` (manual drain trigger). The technician dashboard shows a "X jobs offline pending sync" tile when `pendingCount > 0`.
- **`withOfflineFallback()` wrapper**: Tries the online Firestore call first; only queues on `TypeError` (network failure). Validation errors, permission errors, and other non-network errors are re-thrown normally — they are NOT queued.
- **Device registration is always online**: It requires multiple Firestore reads (device lookup, order lookup) that cannot be deferred, so it skips the offline queue entirely.
- **IndexedDB stores** (`lib/offline/deviceCache.ts`): `devices`, `visits`, `writeQueue`, `photoQueue` — current DB_VERSION=2. If you change the schema, bump the version.
- **Service worker**: Generated by `@serwist/next` at `public/sw.js` (gitignored). Disabled in dev mode. The manifest is in `app/manifest.ts`.
- **Common debugging pitfall**: If a technician reports "action didn't save," check IndexedDB `writeQueue` store in DevTools > Application. Items stuck there indicate the drain failed — check the console for the specific Firestore error on retry.
