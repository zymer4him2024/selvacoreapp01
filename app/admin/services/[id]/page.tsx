...
export default function EditServicePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const svc: Service | null = await getServiceById(serviceId);
        if (cancelled) return;
        if (!svc) {
          setNotFound(true);
          return;
        }
        setName({ en: svc.name?.en ?? '', pt: svc.name?.pt ?? '', es: svc.name?.es ?? '', ko: svc.name?.ko ?? '' });
        setDescription({
          en: svc.description?.en ?? '',
          pt: svc.description?.pt ?? '',
          es: svc.description?.es ?? '',
          ko: svc.description?.ko ?? '',
        });
        setPrice(svc.price != null ? String(svc.price) : '');
        setCurrency(svc.currency || 'USD');
        setDuration(svc.duration != null ? String(svc.duration) : '');
        setCategory(svc.category || '');
        setIncludes(Array.isArray(svc.includes) ? svc.includes : []);
  ...
  if (loading) {
    return (
      <div className="sc" style={{ padding: 32, textAlign: 'center', color: 'var(--soft)' }}>
        Loading service…
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div className="sc" style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'flex-start' }}>
        <h1 className="sc-h1" style={{ margin: 0 }}>Service not found</h1>
  ...
  return (
    <div className="sc" style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div>
          <h1 className="sc-h1" style={{ margin: 0 }}>Edit Service</h1>
          <p className="sc-helper" style={{ margin: '4px 0 0' }}>Update this service&rsquo;s details and pricing.</p>
        </div>
      </div>
